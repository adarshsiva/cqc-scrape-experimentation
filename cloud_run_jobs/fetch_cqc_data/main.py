#!/usr/bin/env python3
"""
Cloud Run Job to fetch CQC data with ratings.
"""
...
class CQCDataFetcher:
    ...
    def save_to_gcs(self, locations: List[Dict]):
        ...
        filename = f"cloud_run_jobs/locations_with_ratings_{timestamp}.json"
        
        blob = self.bucket.blob(filename)
        blob.upload_from_string(json.dumps(locations, indent=2))
        
        logger.info(f"Saved to gs://{self.bucket.name}/{filename}")
        return filename
...
def main():
    ...
    logger.info(f"Cloud Run Job started. Fetching {limit} locations...")
    
    try:
        fetcher = CQCDataFetcher()
        
        # Fetch locations with ratings
        locations = fetcher.fetch_all_locations_with_ratings(limit=limit)
        ...
        if locations:
            ...
            logger.info(f"Data saved to: gs://{fetcher.bucket.name}/{gcs_file}")
        else:
